...
| Web scraping/crawling | | ⚠️ **Update, 2026-08-22**: now present for 5 named sources with no official API (see §2.6 below) — was accurately "not present" at the time this audit row was written |
...
The FastAPI backend's HTTP client (`app/core/http_client.py`) enforces HTTPS-only, a bounded response-size cap (5 MiB default), request timeouts, and capped retry/backoff on 429/5xx — all against a fixed, operator-configured allowlist of official/organization domains, never a user-supplied URL. Ingestion is via each source's official structured API where one exists (Grants.gov Search2, Simpler.Grants.gov, EU Funding & Tenders search API, USAJOBS, ReliefWeb) — the right approach, and preferred over scraping wherever available.

**Update, 2026-08-22:** a web-scraper tier was added (`app/services/web_scraper_base.py` and its subclasses: `cscuk_scholarships.py`, `chevening.py`, `daad_scholarships.py`, `embassy_announcements.py`) for 5 named organizations confirmed to publish no official API, RSS feed, or dataset — see `docs/AUTHORITATIVE_SOURCES.md` #8-#12 for the per-source research, robots.txt/terms check, and page-structure verification against real fetched HTML performed before each was added. This does not weaken the properties above: every scraper request still goes through the same HTTPS-only, size-capped, timeout-bounded, retry-bounded HTTP layer (`app/core/http_client.py::get_html`, added alongside the existing `get_json`), still targets only its own fixed configured domain (never a user-supplied or discovered-at-runtime URL), and additionally rate-limits itself per host (`WebScraperSource.min_request_interval_seconds`, matching or exceeding each site's published `robots.txt` crawl-delay). Every scraped record still lands as `verification_status=pending`/`publication_status=unpublished` — enforced by `NormalizedExternalOpportunity` itself, the same schema every API source produces — so this change affects data *retrieval*, not the trust/approval model audited elsewhere in this document.
